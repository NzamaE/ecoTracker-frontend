...
const SignUp02 = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await withWakeUp(
        (overrides) => API.post("/auth/register", data, overrides),
        {
      ...
      setServerWaking(false);
      setError(err.response?.data?.error || err.message || "Error registering user. Please try again.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <Form {...form}>
          <div className="w-full space-y-4">
            ...
            <Button
              ...
              disabled={loading}
            >
              {serverWaking ? "Waiting for server..." : loading ? "Creating Account..." : "Create Account"}
            </Button>
          </div>
        </Form>
